...
pub fn StatusDialog() -> impl IntoView {
    ...
    let header_str = move || match app_state.status.get() {
        ...
        StatusDialogState::Info(_) => t!(i18n, info)().to_owned(),
        StatusDialogState::Error(_) => t!(i18n, error)().to_owned(),
    };
    let message_str = move || match app_state.status.get() {
        StatusDialogState::None => String::new(),
        StatusDialogState::Loading => t!(i18n, loading)().to_owned(),
        StatusDialogState::Info(ref x) => x.clone(),
        StatusDialogState::Error(ref x) => "❌ ".to_owned() + x,
    };

    view! {
        <Show when=move || !message_str().is_empty() fallback=|| view! {}>
            <div id="dialog">
                <form action="javascript:void(0);" on:submit=move |_| {
                    app_state.status.set(StatusDialogState::None);
                }>
                    <Show when=move || !header_str().is_empty() fallback=|| view! {}>
                        <h3>{header_str}</h3>
                    </Show>
                    <p>{message_str}</p>
                    <Show when=move || app_state.status.get() != StatusDialogState::Loading fallback=|| view! {}>
                        <menu>
